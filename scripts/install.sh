...
# replaced.
#
# Run this as non-root user. You will be prompted for sudo if needed.
#
# Steven P. Goldsmith
# sgjava@gmail.com
# 
...
cp -a "$HOME/u8g2/cppsrc/." "$HOME/java-u8g2/src/main/native-package/src/"
cp -a "$HOME/u8g2/sys/arm-linux/drivers/." "$HOME/java-u8g2/src/main/native-package/src/"
cp -a "$HOME/u8g2/sys/arm-linux/drivers/." "$HOME/java-u8g2/src/main/native-package/src/"
cp -a "$HOME/u8g2/sys/arm-linux/port/." "$HOME/java-u8g2/src/main/native-package/src/"

# Java U8g2 build
cd >> $logfile 2>&1
cd java-u8g2 >> $logfile 2>&1
log "Building Java U8g2..."
mvn clean install -Dmaven.compiler.source=$jdk -Dmaven.compiler.target=$jdk --log-file="scripts/java-u8g2.log" >> $logfile 2>&1
